...
import * as squareDetection from './squareDetection';
import * as triangleDetection from './triangleDetection';
...
function makeCtx(overrides: any = {}) {
    const db = {
        ...
        query: vi.fn((table: string) => ({
            withIndex: () => ({ collect: async () => overrides[table] || [] }),
        })),
    } as any;
...
describe('lineResolver.resolveLineEffects', () => {
    ...
    it('advances turn when no squares completed (triangles ignored for turn)', async () => {
        ...
        const ctx = makeCtx({
            ...
            __newLineKey: '0,1-1,1',
        });

        // Stub shape detectors to return triangle but no squares
        vi.spyOn(squareDetection, 'checkForCompletedSquares').mockResolvedValue([]);
        vi.spyOn(triangleDetection, 'checkForCompletedTriangles').mockResolvedValue(['tri-0,0-BR']);
    ...
    it('updates score and retains turn when square completed and ends game when all squares done', async () => {
        ...
        vi.spyOn(squareDetection, 'checkForCompletedSquares').mockResolvedValue(['0,0']);
        vi.spyOn(triangleDetection, 'checkForCompletedTriangles').mockResolvedValue([]);
