...
import { getRoomByCode, getRoom } from './roomQueries';

function makeDb({ room = null, players = [] } = {}) {
  return {
    query: (tableName: string) => {
      if (tableName === 'rooms') return { withIndex: () => ({ first: async () => room }) } as any;
      if (tableName === 'players') return { withIndex: () => ({ collect: async () => players }) } as any;
      return { withIndex: () => ({ first: async () => null, collect: async () => [] }) } as any;
    },
    get: async (id: any) => (room && room._id === id ? room : null),
  } as any;
}
...
describe('queries/roomQueries', () => {
  ...
  it('getRoomByCode returns room with players sorted', async () => {
    ...
    const players = [ { playerIndex: 1, name: 'B' }, { playerIndex: 0, name: 'A' } ];
  ...
  it('getRoom returns room with players sorted', async () => {
    ...
    const players = [ { playerIndex: 2, name: 'C' }, { playerIndex: 0, name: 'A' }, { playerIndex: 1, name: 'B' } ];
    const ctx: any = { db: makeDb({ room, players }) };
