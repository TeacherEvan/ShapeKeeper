...
function makeDb(player = null) {
  return {
    query: vi.fn(() => ({ withIndex: () => ({ first: async () => player }) })),
...
describe('mutations/toggleReady', () => {
  it('returns error when player not found', async () => {
    ...
    const ctx: any = { db };
    const res = await (toggleReady as any).handler(ctx, { roomId: 'r1', sessionId: 's1' });
  ...
  it('toggles ready state and updates room timestamp', async () => {
    ...
    expect(db.patch).toHaveBeenCalledWith('r1', expect.objectContaining({ updatedAt: expect.any(Number) }));
  });
});