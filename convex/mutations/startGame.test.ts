...
function makeDb({ room = null, players = [] } = {}) {
  return {
    ...
    query: vi.fn((tableName: string) => ({ withIndex: () => ({ collect: async () => players }) })),
...
describe('mutations/startGame', () => {
  ...
  it('starts game when all ready and at least 2 players', async () => {
    const room = { _id: 'r1', hostPlayerId: 'host', status: 'lobby' };
    const players = [{ isReady: true, sessionId: 'p1' }, { isReady: true, sessionId: 'p2' }];
    const db = makeDb({ room, players });
    const ctx: any = { db };

    const res = await (startGame as any).handler(ctx, { roomId: 'r1', sessionId: 'host' });
    expect(res).toEqual({ success: true });
    expect(db.patch).toHaveBeenCalledWith('r1', expect.objectContaining({ status: 'playing' }));
  });
});